...
if(is_array($result)) {
    ...
    foreach ($list_cat_level_1 as $category) {
        if($category["is_visible"]){
            echo "<h4>".$category["level_1"]."</h4>";
        } else{
            echo "<h4><s style='color:#b1b1b1;'>".$category["level_1"]."</s></h4>";
        }
        foreach ($result as $r) {
            if($category["category_level_1_id"] == $r["category_level_1_id"]){
                $category_level_2_id = $r["category_level_2_id"];
                $level_2 = $r["level_2"];
                $is_visible = $r["cat_2_is_visible"];
