...
function addInput($label, $properties, $required = false, $div_class = ""){
    ...
    // pour certain type de champ, même si non obligatoire, ils doivent être corrects
    if($check){
        switch($type){
            case "password":
...
/*
** addSelect est celle de base du projet, mais modifiée à deux endroits:
**      - prise en compte d'une valeur par défaut qui ne serait pas comprise dans la liste d'options reçues
**          => maintenant il y a une option du type "Faites votre choix" qui peut être ajoutée 
**              et qui est de valeur nulle
**      - prise en compte d'une éventuelle valeur entrée par l'utilisateur 
*/
...
function addSelect($label, $properties, $option, $defaultValue, $required = false, $div_class = ""){
    ...
    if(!array_key_exists($defaultValue, $option)){
        $input .= "\t\t\t<option value=\"".null."\"disabled selected>".$defaultValue."</option>\n";
    }

    foreach($option as $key => $value){
        $selected = ($properties["value"] == $key) ? " selected=\"selected\"" : "";
